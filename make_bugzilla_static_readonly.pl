...
sub curl_download {
    ...
    system('curl', @curl_common_args, '-o', $output, $url) == 0 or die("curl failed on $what!");
}
...
for (my $i = 1; $i <= $total_attachments; $i++) {
    ...
    die("Unexpected HTTP response on attachment $i") if (not @httpstatus) or ($httpstatus[-1] !~ /\AHTTP\/\S+\s+200\b/);
